...
/******************************************************************************
 Local variables
 *****************************************************************************/
static AGC_results_t gAgcResults; static AGC_max_results_t gAgcMaxResults = {.IfMaxRx="N/A",
.IfMaxTx = "N/A",
.RfMaxRx = "N/A",
.RfMaxTx="N/A"}; static int gAgcInitialized =0; static int gAgcReady=0; static int gAgcTimeout=0; static Clock_Struct agcClkStruct; static Clock_Handle agcClkHandle; static void processAgcTimeoutCallback(UArg a0);
...
//        return retStatus;
//    }
int mode = scifTaskData.systemAgc.cfg.tddMode; if(mode == 2){ CRS_retVal_t retVal = Tdd_isLocked(); if( retVal == CRS_TDD_NOT_LOCKED){ return retVal; } }
// Clear the ALERT interrupt source
scifClearAlertIntSource();

int i; int j;
// values to correct adc conversion offset
int32_t adcOffset = AUXADCGetAdjustmentOffset(AUXADC_REF_FIXED); int32_t adcGainError = AUXADCGetAdjustmentGain(AUXADC_REF_FIXED); int32_t adcValue,
adcCorrectedValue,
adcValueMicroVolt; uint16_t channelsNum = scifTaskData.systemAgc.cfg.channelsNum;
...
gAgcResults.adcResults[i] = 0; } AGC_results_t newAgcResults = {0};
...
//                j = 0;
//            }
adcSums[0] = scifTaskData.systemAgc.output.channelsMaxRFRX[i]; adcSums[1] = scifTaskData.systemAgc.output.channelsMaxRFTX[i]; adcSums[2] = scifTaskData.systemAgc.output.channelsMaxIFRX[i]; adcSums[3] = scifTaskData.systemAgc.output.channelsMaxIFTX[i]; for(j=0;j<4;j++){
...
adcCorrectedValue = AUXADCAdjustValueForGainAndOffset((int32_t) adcValue, adcGainError, adcOffset); adcValueMicroVolt = AUXADCValueToMicrovolts(AUXADC_FIXED_REF_VOLTAGE_NORMAL,adcCorrectedValue); newAgcResults.adcResults[i+(j*4)] = adcValueMicroVolt; }
...
gAgcResults = newAgcResults; uint32_t randomNumber; TDD_tdArgs_t tdArgs = Tdd_getTdArgs();
// TDD_tdArgs_t tdArgs = Tdd_getTdArgs();
// uint16_t period = tdArgs.period;
// uint16_t dl1 = tdArgs.dl1;
uint16_t period = 5000; uint16_t dl1 = 3857; for(i=0;i<scifTaskData.systemAgc.cfg.samplesCount;i++){
...
randomNumber = (Random_getNumber() % (period - dl1 - 800)); scifTaskData.systemAgc.input.randomDelayTX[i] = randomNumber; dl1 = 1214; randomNumber = (Random_getNumber() % (dl1 - 800)); scifTaskData.systemAgc.input.randomDelayRX[i] = randomNumber; }
...
//        return retStatus;
//    }
int mode = scifTaskData.systemAgc.cfg.tddMode; if(mode == 2){ CRS_retVal_t retVal = Tdd_isLocked(); if( retVal == CRS_TDD_NOT_LOCKED){ AGC_max_results_t gAgcNewResults ={.IfMaxRx="N/A",
.IfMaxTx = "N/A",
.RfMaxRx = "N/A",
.RfMaxTx="N/A"}; gAgcMaxResults = gAgcNewResults;
//CLI_cliPrintf("\r\nSC is not locked");
return retVal; } } if(!gAgcReady){ AGC_max_results_t gAgcNewResults ={.IfMaxRx="N/A",
.IfMaxTx = "N/A", .RfMaxRx = "N/A",
.RfMaxTx="N/A"}; gAgcMaxResults = gAgcNewResults;
...
adcCorrectedValue
    ...
    if (mode == 0 || mode == 2)
    {
        ...
        adcCorrectedValue = AUXADCAdjustValueForGainAndOffset(
                (int32_t) adcValue, adcGainError, adcOffset);
        adcValueMicroVolt = AUXADCValueToMicrovolts(
        AUXADC_FIXED_REF_VOLTAGE_NORMAL,
                                                    adcCorrectedValue);
        //newAgcResults.RfMaxRx = adcValueMicroVolt;
        if (gAgcTimeout
                || strtoul(gAgcMaxResults.RfMaxRx, NULL, 10)
                        < adcValueMicroVolt)
        {
            sprintf(gAgcMaxResults.RfMaxRx, "%i",
                    Agc_convert(adcValueMicroVolt, 0, 0));
        }
        ...
        if (gAgcTimeout
                || strtoul(gAgcMaxResults.IfMaxRx, NULL, 10)
                        < adcValueMicroVolt)
        {
            sprintf(gAgcMaxResults.IfMaxRx, "%i",
                    Agc_convert(adcValueMicroVolt, 0, 1));
        }
    }
    else
    {
    ...
    if (mode == 1 || mode == 2)
    {
        ...
        if (gAgcTimeout
                || strtoul(gAgcMaxResults.RfMaxTx, NULL, 10)
                        < adcValueMicroVolt)
        {
            sprintf(gAgcMaxResults.RfMaxTx, "%i",
                    Agc_convert(adcValueMicroVolt, 1, 0));
        }
        ...
        if (gAgcTimeout
                || strtoul(gAgcMaxResults.IfMaxTx, NULL, 10)
                        < adcValueMicroVolt)
        {
            sprintf(gAgcMaxResults.IfMaxTx, "%i",
                    Agc_convert(adcValueMicroVolt, 1, 1));
        }
    }
    else
    {
...
CRS_retVal_t Agc_setMode(int mode)
{
    ...
    //scifTaskData.systemAgc.state.alertEnabled = 1;
    scifTaskData.systemAgc.cfg.tddMode = mode;
    scifTaskData.systemAgc.cfg.samplesCount =
    scifTaskData.systemAgc.cfg.samplesNum;
